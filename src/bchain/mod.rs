...
impl Blockchain {
    ...
    pub fn append_block(&mut self, block: Block) -> Result<(), String> {
        ...
        if block.get_transaction_count() == 0 {
            return Err(
                "There has to be at least one transaction inside the block! (Code: 9482930)".into(),
            );
        }
        ...
        for (i, transaction) in block.transactions.iter().enumerate() {
            if let Err(err) = transaction.execute(self, &is_genesis) {
                ...
                return Err(format!(
                    "Could not execute transaction {} due to `{}`. Rolling back (Code: 38203984)",
                    i + 1,
                    err
                ));
            }
    ...
    pub fn check_validity(&self) -> Result<(), String> {
        for (block_num, block) in self.blocks.iter().enumerate() {
            ...
            if !block.verify_own_hash() {
                return Err(format!(
                    "Stored hash for Block #{} does not match calculated hash (Code: 665234234)",
                    block_num + 1
                )
                .into());
            }
            ...
            if block_num == 0 {
                ...
                if block.prev_hash.is_some() {
                    return Err("The genesis block has a previous hash set which it shouldn't Code :394823098".into());
                }
            } else {
                ...
                if !(&block.prev_hash == &self.blocks[block_num - 1].hash) {
                    return Err(format!("Block #{} is not connected to previous block (Hashes do not match. Should be `{}` but is `{}`)", block_num, prev_hash_proposed, prev_hash_actual).into());
                }
            }
            ...
            for (transaction_num, transaction) in block.transactions.iter().enumerate() {
                ...
                if transaction.is_signed() && !transaction.check_signature() {
                    return Err(format!(
                        "Transaction #{} for Block #{} has an invalid signature (Code: 4398239048)",
                        transaction_num + 1,
                        block_num + 1
                    ));
                }
